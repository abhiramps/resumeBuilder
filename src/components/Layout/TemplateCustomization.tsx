...
export const TemplateCustomization: React.FC = () => {
    ...
    const resetToTemplateDefaults = () => {
        ...
        dispatch({ type: 'UPDATE_FONT_FAMILY', payload: preset.fontFamily || 'Arial' });
        if (preset.fontSize) {
        ...
        if (preset.lineHeight) {
            dispatch({ type: 'UPDATE_SPACING', payload: { lineHeight: preset.lineHeight } });
        }
    };
        ...
        <div className="space-y-4 p-4 bg-gradient-to-br from-purple-50 to-blue-50 rounded-lg border border-purple-200">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-gray-800">Template Options</h3>
                </div>
                <button
            ...
            {currentTemplate === 'abhiram' && <AbhiramOptions />}
        </div>
    );
};
...
const ClassicOptions: React.FC = () => {
    const { resume, dispatch } = useResumeContext();
    const isSerif = resume.layout.fontFamily?.includes('Times') || resume.layout.fontFamily?.includes('Georgia');

    const toggleFontStyle = () => {
        const newFont = isSerif ? 'Arial, Helvetica, sans-serif' : 'Times New Roman, serif';
        dispatch({ type: 'UPDATE_FONT_FAMILY', payload: newFont });
    };

    const applySpacingVariant = (variant: 'compact' | 'standard' | 'spacious') => {
        const spacingMap = {
    ...
    return (
        <div className="space-y-3">
            <p className="text-xs text-gray-600">Traditional, conservative styling options</p>
            ...
            {/* Spacing Variants */}
            <div className="space-y-2">
                <label className="text-xs font-medium text-gray-700">Spacing Density</label>
                <div className="flex gap-2">
                    <button
...
const ModernOptions: React.FC = () => {
    ...
    return (
        <div className="space-y-3">
            <p className="text-xs text-gray-600">Contemporary design with visual accents</p>
            ...
            <div className="p-3 bg-white rounded border border-gray-300">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-gray-700">Grid Skills Layout</p>
                        <p className="text-xs text-gray-500">Skills displayed in grid format</p>
                    </div>
...
const MinimalOptions: React.FC = () => {
    ...
    const densityLevels = [
        ...
        { name: 'Dense', spacing: 10, lineHeight: 1.2, fontSize: { name: 22, title: 12, sectionHeader: 10, body: 9 } },
        { name: 'Balanced', spacing: 12, lineHeight: 1.3, fontSize: { name: 22, title: 12, sectionHeader: 11, body: 10 } },
    ];

    const applyDensity = (level: typeof densityLevels[0]) => {
        dispatch({ type: 'UPDATE_SPACING', payload: { sectionSpacing: level.spacing, lineHeight: level.lineHeight } });
        dispatch({ type: 'UPDATE_FONT_SIZES', payload: level.fontSize });
    };

    return (
        <div className="space-y-3">
            <p className="text-xs text-gray-600">Space-efficient, maximum content density</p>

            {/* Density Level */}
            <div className="space-y-2">
                <label className="text-xs font-medium text-gray-700">Content Density</label>
                <div className="space-y-2">
                    {densityLevels.map((level) => (
                        ...
                        >
                            ...
                            <div className="text-gray-500">
                                Spacing: {level.spacing}px • Line: {level.lineHeight} • Body: {level.fontSize.body}pt
                            </div>
                        </button>
                    ))}
                </div>
            ...
            <div className="p-3 bg-white rounded border border-gray-300">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-medium text-gray-700">Black & White Only</p>
...
/**
 * Abhiram Template Options
 */
const AbhiramOptions: React.FC = () => {
    const { dispatch } = useResumeContext();

    const colorSchemes = [
        { name: 'Navy Professional', primary: '#2c3e50', secondary: '#555', text: '#333' },
        { name: 'Dark Blue', primary: '#1e3a8a', secondary: '#64748b', text: '#334155' },
        { name: 'Forest Green', primary: '#065f46', secondary: '#6b7280', text: '#374151' },
        { name: 'Deep Purple', primary: '#5b21b6', secondary: '#6b7280', text: '#374151' },
    ];
    ...
    return (
        <div className="space-y-3">
            <p className="text-xs text-gray-600">Professional backend engineer styling</p>
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-2">
                    {colorSchemes.map((scheme) => (
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-medium text-gray-800">{scheme.name}</span>
            ...
            <div className="p-3 bg-white rounded border border-gray-300">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-500">Professional centered layout</p>
                    </div>
